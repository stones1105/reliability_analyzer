...
from qfluentwidgets import (TitleLabel, BodyLabel, PrimaryPushButton, ComboBox,
                            LineEdit, FluentIcon, InfoBar, InfoBarPosition,
                            PushButton, CheckBox, SubtitleLabel)
...
class HCIPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.parent = parent
        self.setObjectName("hciPage")
        self.data_loaded = False
        self.current_file_label_text = ""
        ...
        self.import_btn.clicked.connect(self.parent.load_hci_excel)
        ...
        self.current_btn = PushButton("⚡ 导入三端电流")
        self.current_btn.clicked.connect(self.parent.load_hci_current)
        toolbar_layout.addWidget(self.current_btn)

        self.current_label = QLabel("")
        self.current_label.setStyleSheet("color: #888; font-size: 8pt;")
        toolbar_layout.addWidget(self.current_label)

        toolbar_layout.addStretch(1)  # 弹性间隔

        # ---- 第2组：分析参数 ----
        # 使用 QHBoxLayout 嵌套实现紧凑分组
        ...
        self.slope_combo.addItems(["平均斜率", "独立斜率"])
        ...
        # 退化率
        param_layout.addWidget(BodyLabel("退化率(%):"))
        self.fail_edit = LineEdit()
        self.fail_edit.setText("10")
        self.fail_edit.setMinimumWidth(50)
        self.fail_edit.setAlignment(Qt.AlignCenter)
        param_layout.addWidget(self.fail_edit)

        param_layout.addSpacing(10)

        # 工作电压
        param_layout.addWidget(BodyLabel("工作Vd:"))
        self.vd_edit = LineEdit()
        self.vd_edit.setText("1.1")
        self.vd_edit.setMinimumWidth(55)
        self.vd_edit.setAlignment(Qt.AlignCenter)
        param_layout.addWidget(self.vd_edit)
        ...
        self.target_edit.setMinimumWidth(55)
        ...
        self.model_combo.addItems(["Isub/Id", "1/Vd"])
        ...
        self.analyze_btn.clicked.connect(self.parent.run_hci_analysis)
        ...
        self.detail_table.setColumnCount(13)
        self.detail_table.setHorizontalHeaderLabels(
            ["应力", "器件", "平均斜率(n)", "独立斜率(n)", "TTF(s)", "工作寿命(年)",
             "初始Idsat", "IdOp", "IsubOp", "IgOp", "Idstress", "Isubstress", "Igstress"]
        )
    ...
    def open_preview(self):
        ...
        root = tk.Tk()
        root.withdraw()
        preview = DataPreviewWindow(root, self.parent.hci_data)
        root.wait_window(preview.window)
        points = preview.get_points()
        self.parent.hci_points = points
    ...
    def update_ui(self, res):
        ...
        self.clear_plots()
        fig1 = plot_degradation(res['summary'], res['data'], vg_dict=res.get('vg_dict', {}))
        ...
        fig2 = plot_extrapolation(res['devices_df'], res['result'], res['model_type'])
        ...
        status, detail, color = jedec_judgment(res['result']['tau_years'], res['target_years'])
        text = f"JEDEC 判定: {status}\n{detail}\n\n"
        text += f"外推工作寿命 (t50): {res['result']['tau_years']:.4e} 年\n"
        if 'acceleration_factor' in res['result']:
            text += f"加速因子 AF: {res['result']['acceleration_factor']:.2f}x\n"
        text += f"总器件数: {len(res['tf_work_list'])}\n"
        if cdf_analyzer.results:
            ref = cdf_analyzer.results[0]
            t50_years = ref['t50_years']
            sigma = ref['sigma']
            text += f"\n工作电压下寿命分布:\n"
            text += f"  中位寿命 (t50): {t50_years:.4e} 年\n"
            text += f"  对数标准差 σ: {sigma:.3f}\n"
            z_01 = norm.ppf(0.001)
            t01_years = t50_years * np.exp(sigma * z_01)
            text += f"  t0.1%: {t01_years:.4e} 年\n"
            if t01_years >= res['target_years']:
                text += f"  ✅ t0.1% ≥ {res['target_years']:.2f}年，满足目标\n"
            else:
                text += f"  ❌ t0.1% < {res['target_years']:.2f}年，不满足目标\n"
            q_list = [0.632, 0.50, 0.001, 0.0001, 0.00001]
            labels = ['t63.2%', 't50%', 't0.1%', 't0.01%', 't0.001%']
            for q, lab in zip(q_list, labels):
                if lab == 't0.1%':
                    continue
                z = norm.ppf(q)
                t = t50_years * np.exp(sigma * z)
                text += f"  {lab}: {t:.4e} 年\n"
        else:
            text += "\n（CDF 分析器未返回有效结果）\n"
        ...
        for i, d in enumerate(details):
            ...
            self.detail_table.setItem(i, 6, QTableWidgetItem(f"{d.get('init_idsat', 0):.6e}" if d.get('init_idsat') else ''))
            for j, key in enumerate(['IdOp', 'IsubOp', 'IgOp', 'Idstress', 'Isubstress', 'Igstress'], start=7):
                val = d.get(key, 0)
                self.detail_table.setItem(i, j, QTableWidgetItem(f"{val:.6e}" if val else ''))
